...
def RO_optimization(QD_agent:QDmanager,cluster,Fctrl,target_q:str='q0',run:bool=True,shots:int=10000,plot:bool=True,exp_label=i,IF=xy_IF):
    ...
    info=SS_executor(QD_agent,cluster,Fctrl,target_q,execution=run,shots=shot_num,plot=True if repeat ==1 else False,exp_label=i,IF=xy_IF)#roAmp_modifier=ro_amp_scaling,
    RO_fidelity = info[2]
...
def RO_opti_executor(QD_agent:QDmanager,cluster:Cluster,Fctrl:dict,target_q:str,run:bool=True):#,ro_amp_adj:float=1):
    
    if run:
        RO_optimization(QD_agent,cluster,Fctrl,target_q=target_q,run=run)
    cluster.reset()
...
if __name__ == "__main__":
    ...
    # ro_elements = {'q0':{"ro_amp_factor":1},'q1':{"ro_amp_factor":1}}#
    ro_elements = ['q0']       
    couplers = []
    ...
    for qubit in ro_elements:
        for i in range(repeat):
            ...
            RO_opti_executor(QD_agent,cluster,Fctrl,target_q=qubit,run=execution)
