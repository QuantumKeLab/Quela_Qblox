...
def Zgate_T1(QD_agent:QDmanager,meas_ctrl:MeasurementControl,freeduration:float,Z_amp_min:float,Z_amp_max:float,n_avg:int=500,freeDu_points:int=101,Z_points:int=201,IF:float=150e6,run:bool=True,q='q1',Experi_info:dict={},exp_idx:int=0,no_pi_pulse:bool=False,data_folder:str=''):
    ...
    if run:
        ...
        specific_folder = os.path.join(meas_raw_dir,f"z_gate_T1_pi_{ not no_pi_pulse}")
        if not os.path.exists(specific_folder):
            eyeson_print("New zGate T1 folder built!")
            os.mkdir(specific_folder)
        for i in range(times):
            T1_ds = meas_ctrl.run('Zgate_T1')
            Data_manager().save_raw_data(QD_agent=QD_agent,ds=T1_ds,label=exp_idx,qb=q,exp_type='zT1',specific_dataFolder=specific_folder)
...
if __name__ == "__main__":
    ...
    """ Optional paras """
    flux_span_period_factor = 4 # flux axis = [sweet-period/flux_span_period_factor, sweet+period/flux_span_period_factor]
    flux_data_points = 300
