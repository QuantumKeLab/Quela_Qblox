...
def correlation_method(ave_Ve_nc_file, Vk_directory, f01, correlate_delays, plot_title):
    ...
    return plot_title, correlate_delays, g1_values_per_group, g1_std_per_group, ans, fitted_info

def analyze_all_folders(root_directory, f01, correlate_delays):
    all_results = []
    color_map = cm.get_cmap('hsv', len(os.listdir(root_directory)))  # Get a color map with enough colors for each dataset
    ...
    # Sort all results by the numeric value in the plot title (assuming temperature in mK is in the title)
    all_results.sort(key=lambda x: int(re.search(r'(\d+)', x[0]).group()))

    # Plot all results in one figure
    plt.figure(figsize=(12, 8))
    for idx, (plot_title, delays, g1_values, g1_std, ans, fitted_info) in enumerate(all_results):
        color = color_map(idx / len(all_results))  # Get a color from the color map for each dataset
        plt.errorbar(delays, g1_values, yerr=g1_std, marker='o', linestyle='none', capsize=5, color=color, label=f"{plot_title} Data")
        plt.plot(delays, t1(delays, *ans), color=color, linewidth=2, linestyle='-', label=f"{plot_title} Fit")
    ...
    # temperature_values = [int(re.search(r'(\d+)', x[0]).group()) for x in all_results]
    # delays_grid, temps_grid = np.meshgrid(correlate_delays, temperature_values)
    # g1_values_grid = np.array([x[2] for x in all_results])
    
    # # Adjust g1_values_grid to match the dimensions of the meshgrid
    # if g1_values_grid.shape[1] > len(correlate_delays):
    #     g1_values_grid = g1_values_grid[:, :len(correlate_delays)]

    # plt.pcolormesh(delays_grid, temps_grid, g1_values_grid, shading='nearest', cmap='viridis')
    # plt.colorbar(label='Average g^(1)')
    # plt.xlabel("Correlate Delay (us)")
    # plt.ylabel("Temperature (mK)")
    # plt.title("2D Heatmap of Average g^(1) vs Delay and Temperature")
    # plt.show()
    
if __name__ == '__main__':
    root_directory = r"C:\Users\admin\SynologyDrive\09 Data\Fridge Data\Qubit\20241024_DRKe_5XQv4#5_second_coating_and_effT\Meas_raw\Q3_CopyFoldersForMainAnalysis"
