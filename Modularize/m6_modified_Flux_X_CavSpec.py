...
def Flux_X_Cav_spec(QD_agent:QDmanager,meas_ctrl:MeasurementControl,flux_ctrl:dict,ro_span_Hz:int=3e6,flux_span:float=0.3,n_avg:int=300,f_points:int=20,flux_points:int=20,run:bool=True,q:str='q1',z:str='q1',Experi_info:dict={}):
    ...
    if run:
        ...
        meas_ctrl.settables([freq,flux_ctrl[z]])
        ...
        update_2Ddict(analysis_result, q, z, ResonatorFluxSpectroscopyAnalysis(tuid=rfs_ds.attrs["tuid"], dataset=rfs_ds).run())
...
def flux_X_Cavity_executor(QD_agent:QDmanager,meas_ctrl:MeasurementControl,specific_qubits:str,specific_zlines:str,run:bool=True,flux_span:float=0.4,ro_span_Hz=3e6,zpts=20,fpts=30):
    FD_results = {}
    if run:
        print(f"{specific_qubits} are under the {specific_zlines} measurement ...")
        update_2Ddict(FD_results, specific_qubits, specific_zlines, Flux_X_Cav_spec(QD_agent,meas_ctrl,Fctrl,ro_span_Hz=ro_span_Hz,q=specific_qubits,z=specific_zlines,flux_span=flux_span,flux_points=zpts,f_points=fpts)[specific_qubits][specific_zlines])
        if FD_results == {}:
...
if __name__ == "__main__":
    ...
    update = False
    FD_results = {}
    Z_matrix=np.zeros((q_num,q_num))
    normalized_Z=np.zeros((q_num,q_num))
    for i, qubit in enumerate(ro_elements):
        for j, zline in enumerate(ro_elements):
            ...
            update_2Ddict(FD_results, qubit, zline, flux_X_Cavity_executor(QD_agent,meas_ctrl,qubit,zline,run=execution,flux_span=0.8,ro_span_Hz=12e6, zpts=30)[qubit][zline])
            Z_matrix[i][j] = FD_results[qubit][zline].quantities_of_interest["frequency"].nominal_value/(2*pi)
            cluster.reset()
            ...
            #     break
    print(Z_matrix)
    diagonalized_Z=np.diag(Z_matrix)
    print(diagonalized_Z)
    for i in range(len(ro_elements)):
        for j in range(len(ro_elements)):
            normalized_Z[i,j]=Z_matrix[i,j]/diagonalized_Z[i]
    print(normalized_Z) 
    inv_Z = np.linalg.inv(normalized_Z)
    print(inv_Z)
