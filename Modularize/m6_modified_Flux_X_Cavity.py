...
# def fluxCavity_executor(QD_agent:QDmanager,meas_ctrl:MeasurementControl,specific_qubits:str,run:bool=True,flux_span:float=0.4,ro_span_Hz=3e6,z_pts:int=20):
...
#         FD_results = FluxCav_spec(QD_agent,meas_ctrl,Fctrl,ro_span_Hz=ro_span_Hz,q=specific_qubits,flux_span=flux_span,flux_points=z_pts)[specific_qubits]
#         if FD_results == {}:
...
#         FD_results = FluxCav_spec(QD_agent,meas_ctrl,Fctrl,ro_span_Hz=ro_span_Hz,q=specific_qubits,flux_span=flux_span,run=False,flux_points=z_pts)

#     return FD_results

def flux_X_Cavity_executor(QD_agent:QDmanager,meas_ctrl:MeasurementControl,specific_qubits:str,specific_zline:str,run:bool=True,flux_span:float=0.4,ro_span_Hz=3e6,z_pts:int=20):
...
if __name__ == "__main__":
    ...
    update = False
    FD_results = {}
    for zline in ro_elements:
        for qubit in ro_elements:
            FD_results[qubit] = flux_X_Cavity_executor(QD_agent,meas_ctrl,qubit,zline, flux_span=0.15, ro_span_Hz=4e6,run=execution,z_pts=40)
            cluster.reset()
            if execution:
