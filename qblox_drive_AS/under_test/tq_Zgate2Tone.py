...
def Two_tone_spec(QD_agent:QDmanager,meas_ctrl:MeasurementControl,IF:float=200e6,f01_guess:int=0,Z_amp:float=0,xyf_span_Hz:int=400e6,xyamp:float=0.02,n_avg:int=500,points:int=200,run:bool=True,q:str='q1',Experi_info:dict={},ref_IQ:list=[0,0],drive_read_overlap:bool=False):
    ...
    if not drive_read_overlap:
        ...
        qubit_info.reset.duration(1010e-6)
        drive_pulse_length = 1000e-6
    else:
...
def conti2tone_executor(QD_agent:QDmanager,meas_ctrl:MeasurementControl,cluster:Cluster,specific_qubits:str,Z_amp:float,XYF:float,XYL:float,xyf_span:float=500e6,xy_if:float=200e6,run:bool=True,V_away_from:float=0,drive_read_overlap:bool=False,avg_times:int=500):
    ...
    if run:
        ...
        QS_results = Two_tone_spec(QD_agent,meas_ctrl,Z_amp=Z_amp,xyamp=XYL,IF=xy_if,f01_guess=XYF,q=specific_qubits,xyf_span_Hz=xyf_span,points=200,n_avg=1000,run=True,ref_IQ=QD_agent.refIQ[specific_qubits],drive_read_overlap=drive_read_overlap) # 
        ...
        qu = specific_qubits
        QS_results = Two_tone_spec(QD_agent,meas_ctrl,Z_amp=Z_amp,xyamp=0.1,IF=xy_if,f01_guess=4e9,q=qu,xyf_span_Hz=xyf_span,points=50,n_avg=avg_times,run=False,ref_IQ=QD_agent.refIQ[qu],drive_read_overlap=drive_read_overlap)
...
if __name__ == "__main__":
    ...
    #
    ro_elements = {
        "q0":{"xyf_guess":[5.0e9],"xyl_guess":[0.03],"g_guess":100e6, "tune_bias":0} # g you can try a single value about 90e6 for a 5Q4C chip.
    ...
    for qubit in ro_elements:
        ...
        for xyf in fq_guesses:
            for xyl in xyl_guesses: 
                ...
                print(xyf)
                tt_results = conti2tone_executor(QD_agent,meas_ctrl,cluster,specific_qubits=qubit,Z_amp=Z_amp,XYF=xyf,XYL=xyl,run=execution,xy_if=100e6,xyf_span=xyf_range,V_away_from=tune_bias,drive_read_overlap=drive_read_overlap,avg_times=avg_n)
