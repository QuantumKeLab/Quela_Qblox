...
def correlation_method(ave_Ve_nc_file, Vk_directory, f01, correlate_delays, plot_title):
    ...
    # Step 6: Plot averaged results with fitted curve
    plt.figure(figsize=(10, 6))
    plt.suptitle(fitted_info, fontsize=10)
    plt.title(plot_title)
    plt.errorbar(correlate_delays, g1_values_per_group, yerr=g1_std_per_group, marker='o', linestyle='none',color='deepskyblue', capsize=5,label="Average g^(1) per group of three files")
    plt.plot(correlate_delays, t1(correlate_delays, *ans), c='dodgerblue', linewidth=3, label="T1 Fit")
